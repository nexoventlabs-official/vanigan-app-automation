...
export default function Postings() {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error(err);
      setError('Failed to load postings list.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = async (e) => {
    ...
    setError('');
    try {
      await api.post('/postings', { name: name.trim() });
      setName('');
      await load();
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to add posting.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id, nameVal) => {
    if (!confirm(`Delete posting "${nameVal}"? This might affect new organizer selections.`)) return;
    setError('');
    try {
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight flex items-center gap-2">
            <Shield className="text-[#66ff4c]" />
            Organizer Postings / Roles
          </h1>
          <p className="text-sm text-gray-400 mt-1 font-semibold">
            Manage the titles and roles available for organizers in the Direct Creator form.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
        ...
        <div className="md:col-span-8 card overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-500 text-sm">No postings added yet. Add one on the left.</div>
          ) : (
