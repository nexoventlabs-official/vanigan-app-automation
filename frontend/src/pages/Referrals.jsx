...
export default function Referrals() {
  ...
  return (
    <div className="p-4 lg:p-6 max-w-7xl mx-auto h-[calc(100vh-80px)] lg:h-[calc(100vh-40px)] flex flex-col gap-6">
      
      {/* Header section */}
      <div className="shrink-0 flex items-center justify-between gap-4 flex-wrap border-b border-white/[0.08] pb-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white flex items-center gap-2">
            <Share2 size={24} className="text-[#66ff4c]" />
            References &amp; Networks
          </h1>
        ...
        <div className="flex gap-2">
          <div className="bg-[#0A0E17] border border-white/[0.08] rounded-lg p-0.5 flex">
            <button 
              onClick={() => setSortBy('name')} 
              className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${sortBy === 'name' ? 'bg-[#66ff4c]/10 text-[#66ff4c]' : 'text-gray-400 hover:text-white'}`}
            >
              Name
            </button>
            <button 
              onClick={() => setSortBy('count')} 
              className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${sortBy === 'count' ? 'bg-[#66ff4c]/10 text-[#66ff4c]' : 'text-gray-400 hover:text-white'}`}
            >
              Referral Count
            </button>
          </div>
      ...
      <div className="flex-1 flex gap-6 overflow-hidden min-h-0 relative">
        ...
        <div className={`w-full lg:w-[380px] bg-[#06080D] border border-white/[0.08] rounded-2xl flex flex-col overflow-hidden shrink-0 ${selectedMemberId ? 'hidden lg:flex' : 'flex'}`}>
          ...
          <div className="p-4 border-b border-white/[0.08] space-y-3">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-2 bg-white/5 border border-white/10 rounded-xl text-xs text-white placeholder-gray-500 focus:outline-none focus:border-[#66ff4c]" 
              />
            </div>
            
            {/* Filters */}
            <div className="flex gap-1 bg-[#000000]/60 p-0.5 rounded-lg border border-white/[0.04]">
              {['all', 'referrers', 'referred'].map((type) => (
                <button
                  ...
                  className={`flex-1 text-center py-1 rounded text-[10px] uppercase font-bold tracking-wider transition-all ${
                    filterType === type 
                      ? 'bg-[#66ff4c]/15 text-[#66ff4c] border border-[#66ff4c]/20' 
              ...
              filteredAndSortedMembers.map((m) => {
                ...
                return (
                  <div
                    ...
                    className={`p-3.5 flex items-center gap-3 hover:bg-white/[0.02] cursor-pointer transition-all duration-150 ${
                      isActive 
                        ? 'bg-[#66ff4c]/5 border-l-4 border-[#66ff4c]' 
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs font-extrabold text-white truncate leading-snug">
                          {m.name}
                        </span>
                        {m.isOrganizer && (
                    ...
                    {m.referralCount > 0 ? (
                      <div className="shrink-0 flex items-center gap-1 px-2 py-1 rounded bg-[#66ff4c]/10 border border-[#66ff4c]/20 text-[#66ff4c] text-[10px] font-mono font-black">
                        <Users size={10} className="stroke-[2]" />
                        {m.referralCount}
                      </div>
                    ) : (
                      <div className="shrink-0 text-[10px] text-gray-600 font-mono">0 ref</div>
                    )}
                  </div>
                );
              })
        ...
        <div className={`flex-1 bg-[#0A0E17] border border-white/[0.08] rounded-2xl flex flex-col overflow-hidden ${!selectedMemberId ? 'hidden lg:flex justify-center items-center p-8' : 'flex'}`}>
          
          {!selectedMember ? (
            // Empty State
            <div className="text-center space-y-4 max-w-sm">
              <div className="w-16 h-16 rounded-2xl bg-[#66ff4c]/5 text-[#66ff4c] border border-[#66ff4c]/25 flex items-center justify-center mx-auto shadow-[0_0_20px_rgba(102,255,76,0.1)]">
                <Share2 size={24} className="stroke-[1.5]" />
              </div>
              <div className="space-y-1">
                <h3 className="text-sm font-extrabold text-white">Select a Member</h3>
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-y-auto">
              ...
              {/* Mobile Back Button */}
              <div className="p-4 border-b border-white/[0.08] flex items-center justify-between shrink-0 lg:hidden">
                <button 
                ...
                <span className="text-[10px] uppercase font-black tracking-widest text-[#66ff4c] bg-[#66ff4c]/10 border border-[#66ff4c]/20 px-2 py-0.5 rounded">
                  Reference Detail
                </span>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-start gap-5 flex-wrap md:flex-nowrap pb-6 border-b border-white/[0.08]">
                  {/* Big Image/Avatar */}
                  {selectedMember.photoUrl ? (
                    <img 
                      src={selectedMember.photoUrl} 
                      alt={selectedMember.name} 
                      className="w-20 h-20 rounded-2xl object-cover border-2 border-[#66ff4c]/30 shadow-lg" 
                    />
                  ) : (
                  ...
                  <div className="space-y-2 flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {selectedMember.isOrganizer ? (
                        ...
                        <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-white/5 border border-white/10 text-gray-450 uppercase tracking-widest">
                          Member
                        </span>
                      )}
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1.5 text-xs text-gray-400">
                      ...
                      {selectedMember.district && (
                        <div className="flex items-center gap-2">
                          <Grid size={13} className="text-gray-500 shrink-0" />
                      ...
                      {selectedMember.assemblyName && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-white">Assembly:</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  {/* Referral Code Card */}
                  <div className="bg-[#06080D] border border-white/[0.08] rounded-xl p-4 flex flex-col justify-between min-h-[90px]">
                    <div className="text-[10px] uppercase font-bold text-gray-500 tracking-wider flex items-center gap-1.5">
                      <LinkIcon size={12} className="text-gray-400" />
                      Referral Code
                    </div>
                    <div className="text-sm font-extrabold text-white mt-2 font-mono tracking-wide">
                  ...
                  <div className="bg-[#06080D] border border-white/[0.08] rounded-xl p-4 flex flex-col justify-between min-h-[90px]">
                    ...
                    {referrerOfSelected ? (
                      <div 
                        ...
                        className="group mt-2 flex items-center gap-2 cursor-pointer hover:text-[#66ff4c] transition"
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <div className="text-xs font-extrabold truncate text-white group-hover:text-[#66ff4c] transition">{referrerOfSelected.name}</div>
                          <div className="text-[9px] text-gray-500 font-mono mt-0.5 truncate">{referrerOfSelected.membershipId}</div>
                        </div>
                      </div>
                    ) : (
                  ...
                  <div className="bg-[#06080D] border border-white/[0.08] rounded-xl p-4 flex flex-col justify-between min-h-[90px]">
                    ...
                    <div className="text-xl font-mono font-black text-[#66ff4c] mt-2">
                ...
                <div className="space-y-3">
                  <h3 className="text-xs uppercase tracking-wider font-extrabold text-gray-400 flex items-center gap-1.5">
                    <Share2 size={13} className="text-[#66ff4c]" />
                    Referred Network ({downstreamReferrals.length})
                  </h3>

                  {downstreamReferrals.length === 0 ? (
                    <div className="bg-[#06080D] border border-dashed border-white/10 rounded-xl p-8 text-center text-xs text-gray-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {downstreamReferrals.map((child) => (
                        <div 
                          ...
                          className="flex items-center gap-3 p-3 bg-[#06080D] hover:bg-[#66ff4c]/5 border border-white/[0.06] hover:border-[#66ff4c]/20 rounded-xl cursor-pointer transition duration-150 group"
                        >
                          {/* Child Avatar */}
                          {child.photoUrl ? (
                            <img 
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-xs font-extrabold text-white group-hover:text-[#66ff4c] transition truncate">{child.name}</div>
                            <div className="text-[9px] text-gray-500 font-mono mt-0.5 truncate">{child.membershipId}</div>
                          </div>

                          <ChevronRight size={14} className="text-gray-600 group-hover:text-[#66ff4c] transition-colors" />
                        </div>
                      ))}
                    </div>
                  )}
