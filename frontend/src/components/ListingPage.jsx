...
import { Plus, Pencil, Trash2, Image as ImageIcon, X, Search, ChevronLeft, ChevronRight, Phone, Mail, Globe, MapPin, Clock, Tag, ExternalLink } from 'lucide-react';
import api from '../api';
    ...
    <div className="space-y-4">
        ...
        <div className="card overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {items.map((it, idx) => (
                <tr key={it._id} className="hover:bg-orange-50/40 transition-colors cursor-pointer" onClick={() => setViewItem(it)}>
      ...
      {viewItem && (
        <div className="fixed inset-0 z-40 flex">
          <div className="flex-1 bg-black/40" onClick={() => setViewItem(null)} />
          <div className="w-full max-w-md bg-white shadow-2xl flex flex-col overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-brand-700 text-white">
              <div className="flex items-center gap-3 min-w-0">
                {viewItem.image ? (
                  <img src={viewItem.image} alt="" className="w-10 h-10 rounded-lg object-cover flex-shrink-0" />
                ) : (
                ...
                <div className="min-w-0">
                  <div className="font-bold text-base truncate">{viewItem.name}</div>
                  <div className="text-xs text-white/70">{viewItem.listingCode || ''}</div>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className={`pill text-xs ${viewItem.active ? 'bg-green-200 text-green-800' : 'bg-gray-200 text-gray-600'}`}>
                  {viewItem.active ? 'Active' : 'Inactive'}
                </span>
                <button onClick={() => setViewItem(null)} className="text-white/80 hover:text-white ml-1">
                  <X size={20} />
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              {viewItem.category && (
                <div className="flex items-start gap-2">
                  <Tag size={15} className="text-brand-500 mt-0.5 flex-shrink-0" />
                  <span className="text-sm text-gray-700">{viewItem.category}</span>
                </div>
              )}
              ...
              {viewItem.address && (
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <div className="text-sm text-gray-700">{viewItem.address}</div>
                    {viewItem.landmark && <div className="text-xs text-gray-400 mt-0.5">📍 {viewItem.landmark}</div>}
                  </div>
                </div>
              )}

              {/* District / Assembly */}
              {(viewItem.district || viewItem.assembly) && (
                <div className="flex gap-2 flex-wrap">
                  {viewItem.district && <span className="pill bg-orange-50 text-orange-700 text-xs">{viewItem.district}</span>}
                  {viewItem.assembly && <span className="pill bg-orange-50 text-orange-700 text-xs">{viewItem.assembly}</span>}
                </div>
              )}

              {/* Phone */}
              {(viewItem.phone || viewItem.phone2) && (
                <div className="flex items-start gap-2">
                  <Phone size={15} className="text-brand-500 mt-0.5 flex-shrink-0" />
                  <div className="text-sm text-gray-700 space-y-0.5">
                    {viewItem.phone && <div><a href={`tel:${viewItem.phone}`} className="hover:text-brand-700">{viewItem.phone}</a></div>}
                    {viewItem.phone2 && <div className="text-gray-500"><a href={`tel:${viewItem.phone2}`} className="hover:text-brand-700">{viewItem.phone2}</a></div>}
                  </div>
                </div>
              )}
              ...
              {viewItem.email && (
                <div className="flex items-center gap-2">
                  ...
                  <a href={`mailto:${viewItem.email}`} className="text-sm text-gray-700 hover:text-brand-700 truncate">{viewItem.email}</a>
                </div>
              )}
              ...
              {viewItem.website && (
                <div className="flex items-center gap-2">
                  <Globe size={15} className="text-brand-500 flex-shrink-0" />
                  <a href={viewItem.website} target="_blank" rel="noreferrer" className="text-sm text-brand-700 hover:underline truncate flex items-center gap-1">
                    {viewItem.website} <ExternalLink size={11} />
                  </a>
                </div>
              )}

              {/* Hours */}
              {(viewItem.openDays || viewItem.openTime || viewItem.closeTime) && (
                <div className="flex items-start gap-2">
                  <Clock size={15} className="text-brand-500 mt-0.5 flex-shrink-0" />
                  <div className="text-sm text-gray-700">
                    {viewItem.openDays && <div className="text-xs text-gray-500 mb-0.5">{viewItem.openDays}</div>}
                    {(viewItem.openTime || viewItem.closeTime) && (
                      <div>{viewItem.openTime || '—'} – {viewItem.closeTime || '—'}</div>
                    )}
                  </div>
                </div>
              )}
              ...
              {/* Map link */}
              {viewItem.lat && viewItem.lng && (
                <a
                  href={`https://maps.google.com/?q=${viewItem.lat},${viewItem.lng}`}
              ...
              {viewItem.ownerPhone && (
                <div className="border-t border-gray-100 pt-3 text-xs text-gray-400">
                  Registered by: <span className="text-gray-600">{viewItem.ownerPhone}</span>
                </div>
              )}
            </div>
            ...
            <div className="px-5 py-3 border-t border-gray-100 flex gap-2">
              ...
              <button
                onClick={() => { if (confirm('Delete this listing?')) { remove(viewItem._id); setViewItem(null); } }}
