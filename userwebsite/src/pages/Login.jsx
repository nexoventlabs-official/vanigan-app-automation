  ...
  return (
    <div className="container section" style={{ maxWidth: 440 }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-pp-neue-montreal)', fontSize: '30px', fontWeight: 700, letterSpacing: '-0.02em', color: 'var(--color-rich-black)', marginBottom: 6 }}>Welcome Back</h1>
        <p style={{ fontFamily: 'var(--font-pp-neue-montreal)', color: 'var(--color-cool-gray)', fontSize: '14px', margin: 0 }}>
          {step === 'phone' ? 'Enter your registered mobile number.' : `Enter your PIN for ${phone.replace(/\D/g,'').slice(0,3)}…${phone.replace(/\D/g,'').slice(-3)}`}
        </p>
      </div>
      ...
      {step === 'phone' && (
        <div className="card" style={{ padding: 28, background: 'var(--color-canvas-white)', borderRadius: 12, border: '1px solid var(--color-subtle-ash)' }}>
          ...
          <p style={{ textAlign: 'center', marginTop: 20, fontFamily: 'var(--font-pp-neue-montreal)', fontSize: '13px', color: 'var(--color-cool-gray)' }}>
            New to Vanigan? <button type="button" onClick={() => navigate('signup')} style={{ background: 'none', border: 'none', color: 'var(--color-deep-fern-green)', cursor: 'pointer', fontWeight: 600, fontSize: '13px', padding: 0 }}>Sign Up</button>
          </p>
        </div>
      )}
      ...
      {step === 'pin' && (
        <div className="card" style={{ padding: 28, background: 'var(--color-canvas-white)', borderRadius: 12, border: '1px solid var(--color-subtle-ash)' }}>
          <div style={{ fontFamily: 'var(--font-pp-neue-montreal)', fontSize: '13px', color: 'var(--color-cool-gray)', marginBottom: 24, textAlign: 'center' }}>
          ...
          <p style={{ textAlign: 'center', marginTop: 20, fontFamily: 'var(--font-pp-neue-montreal)', fontSize: '13px', color: 'var(--color-cool-gray)' }}>
            Don't have an account? <button type="button" onClick={() => navigate('signup')} style={{ background: 'none', border: 'none', color: 'var(--color-deep-fern-green)', cursor: 'pointer', fontWeight: 600, fontSize: '13px', padding: 0 }}>Sign Up</button>
          </p>
        </div>
      )}
    </div>
