...
export default function MyBusiness() {
  ...
  const [step, setStep] = useState(() =>
    isLoggedIn && sessionBiz ? "view" : isLoggedIn ? "no_business" : "phone",
  );
  // True while we're doing the one-time API check for a freshly linked business
  ...
  // On mount: if logged in but no business in cached session, check the API
  // once. This handles the case where the user linked their business via the
  // backend registration + PIN setup page (new tab) — the localStorage session
  // is stale but the DB already has member.businessId set.
  useEffect(() => {
    if (!isLoggedIn || sessionBiz || !user?.phone) return;
    let cancelled = false;
    setCheckingBiz(true);
    (async () => {
      try {
        ...
        /* silent — fall through to no_business UI */
      } finally {
        if (!cancelled) setCheckingBiz(false);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const handlePinSubmit = async (e) => {
    ...
    try {
      ...
      if (code === "wrong_pin") setPinError("Incorrect PIN. Please try again.");
        ...
        setPinError(
          "No PIN has been set for this business yet. Please complete registration.",
        );
  ...
  /* One-time API check in progress — show a brief loading state so the
     user never sees a flash of "Add Your Business" when the business was
     just linked via the backend registration tab. */
  if (checkingBiz) {
    return (
      <div
        className="container section"
        style={{ maxWidth: 480, textAlign: "center", paddingTop: 60 }}
      >
  ...
  if (
    step === "no_business" ||
    (isLoggedIn && !biz && step !== "phone" && step !== "pin")
  ) {
    return (
      <div
        className="container section"
      ...
      >
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          >
            Hi <strong>{user?.name}</strong>, you haven't listed a business yet.
          </p>
        </div>
        <div
          className="card"
        ...
        >
          ...
          >
            Get your business discovered by customers across Tamil Nadu. List it
            for free!
          </p>
          <button
    ...
    <div>
        ...
        <div className="biz-cover-container">
          {biz.coverImage ? (
            ...
            <div className="biz-cover-placeholder">
              <Store
                size={48}
                style={{ color: "var(--color-cool-gray)", opacity: 0.6 }}
              />
            </div>
          )}
        </div>
      ...
      <div className="container" style={{ paddingTop: 0 }}>
                ...
                >
                  ...
                  >
                    ...
                    {biz.active && (
                      ...
                      >
                        <path
                          d="M22.5 12.5c0-1.58-.875-2.95-2.148-3.6.154-.435.238-.905.238-1.4 0-2.21-1.71-3.99-3.818-3.99-.48 0-.941.1-1.358.275C14.77 2.515 13.498 1.5 12 1.5s-2.77 1.015-3.412 2.285c-.417-.175-.878-.275-1.358-.275-2.108 0-3.818 1.78-3.818 3.99 0 .495.084.965.238 1.4-1.273.65-2.148 2.02-2.148 3.6 0 1.58.875 2.95 2.148 3.6-.154.435-.238.905-.238 1.4 0 2.21 1.71 3.99 3.818 3.99.48 0 .941-.1 1.358-.275.642 1.27 1.914 2.285 3.412 2.285s2.77-1.015 3.412-2.285c.417.175.878.275 1.358.275 2.108 0 3.818-1.78 3.818-3.99 0-.495-.084-.965-.238-1.4 1.273-.65 2.148-2.02 2.148-3.6z"
                          fill="#0095F6"
                        />
                        <path
                          d="M9.78 16.72l-3.86-3.86 1.41-1.41 2.45 2.45 6.18-6.18 1.41 1.41-7.59 7.59z"
                          fill="white"
                        />
                      </svg>
                    )}
                  ...
                  <span
                    className={`badge ${biz.active ? "badge-green" : "badge-gray"}`}
                    style={{
                      display: "inline-flex",
                      alignItems: "center",
                      whiteSpace: "nowrap",
          ...
          >
            ...
            <button
              ...
              style={{
                paddingInline: 14,
                borderRadius: "12px",
                borderColor: "var(--color-subtle-ash)",
                fontSize: "12px",
                color: "var(--color-cool-gray)",
                display: "flex",
                alignItems: "center",
        ...
        {!biz.active && (
          ...
          >
            ⏳ Your business is pending review. Our team will activate it
            shortly.
          </div>
        )}
        ...
        >
          <div>
            ...
            >
              View your business listing as customers see it or preview on
              WhatsApp
            </p>
          </div>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button
              onClick={() => navigate("detail", { id: biz._id })}
              className="btn btn-primary btn-sm"
              style={{ paddingInline: 16, borderRadius: "12px" }}
            >
              <ExternalLink size={13} /> View Public Listing
            </button>
            {phone && (
              <a
        ...
        <div className="biz-detail-grid">
          <div>
            ...
            <InfoSection
              title={`Customer Reviews (${(biz.reviews || []).length})`}
            >
              {!biz.reviews || biz.reviews.length === 0 ? (
                <div
                  style={{
                    background: "var(--color-canvas-white)",
                    border: "1px dashed var(--color-subtle-ash)",
                    borderRadius: "12px",
                    padding: "24px",
                    textAlign: "center",
                    fontFamily: "var(--font-pp-neue-montreal)",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <SideCard title="Location">
              {[
                ...
                biz.serviceLocations && {
                  icon: Tag,
                  label: "Service Areas",
                  value: biz.serviceLocations,
                },
              ]
                .filter(Boolean)
                .map((r, i) => (
            ...
            <div className="dialog-body">
              ...
              >
                ...
                {selectedService.price && (
                  <div className="dialog-price">₹ {selectedService.price}</div>
                )}
                {selectedService.detail && (
                  <p className="dialog-desc">{selectedService.detail}</p>
                )}
              </div>
            </div>
          </div>
...
const SUB_CATEGORIES = {
  ...
  "Hire Services": [
    ...
    "Event Equipment (DJ/Sound/Projector)",
    "Furniture & Appliances on Hire",
    "Costumes & Formal Wear on Hire",
    "Construction Equipment on Hire",
    "Cooking & Catering Staff on Hire",
  ],
  Automobile: [
    "Car & Bike Sales",
  ...
  Repairs: [
    ...
    "Two-Wheeler & Car Repair",
    "CCTV & Generator Repair",
  ],
  "IT & Software": [
  ...
  Religious: [
    ...
    "Religious Product Dealers",
    "Religious Audio & DVD Dealers",
    "Temple Construction",
    "Pooja Item Shops",
  ],
  "Organic Products": [
...
function EditBusiness({ biz, ownerPhone, pin, onSave, onCancel }) {
  ...
  const handleGalleryChange = (e) => {
    ...
    files.forEach((f) => {
      const reader = new FileReader();
      reader.onload = (ev) =>
        setGalleryPreviews((p) => [...p, ev.target.result]);
      reader.readAsDataURL(f);
    });
  };
  ...
  const handleServiceImage = (i, e) => {
    ...
    const reader = new FileReader();
    reader.onload = (ev) =>
      updateService(i, "newImagePreview", ev.target.result);
    reader.readAsDataURL(f);
  ...
  const handleSubmit = async (e) => {
    ...
    if (cleanNew || cleanConfirm) {
      ...
      if (cleanConfirm.length < 4) {
        setPinChangeError("Please confirm your new PIN.");
        return;
      }
      if (cleanNew !== cleanConfirm) {
    ...
    if (galleryToRemove.length)
      fd.append("galleryToRemove", galleryToRemove.join(","));
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          ...
          >
            ...
            >
              Modify your business name, category and description
            </p>
          </div>
        ...
        >
          ...
          >
            ...
            >
              Update your physical location details
            </p>
          </div>
          <div style={fieldStyle}>
          ...
          <div style={fieldStyle}>
            <label style={labelStyle}>Service Locations</label>
            <input
              style={inputStyle}
              value={form.serviceLocations}
              onChange={(e) => setField("serviceLocations", e.target.value)}
            />
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            >
              Update phone numbers, email, and website
            </p>
          </div>
          <div
            style={{
          ...
          >
            ...
            <div>
              <label style={labelStyle}>Alt Phone</label>
              <input
                style={inputStyle}
                value={form.phone2}
        ...
        >
          ...
          >
            ...
            >
              Specify opening days and timing schedules
            </p>
          </div>
              ...
              {DAYS.map((d) => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: "12px",
                    fontWeight: 500,
                    border: `1px solid ${openDays.includes(d) ? "var(--color-ink)" : "var(--border)"}`,
                    background: openDays.includes(d)
                      ? "var(--color-ink)"
                      : "transparent",
                    color: openDays.includes(d)
                      ? "var(--color-snow)"
                      : "var(--color-graphite)",
                    cursor: "pointer",
                    transition: "all .15s",
        ...
        >
          ...
          >
            ...
            >
              List key services/products offered (max 6)
            </p>
          </div>
          {services.map((s, i) => (
            <div
            ...
            >
              ...
              <div>
                ...
                >
                  <Camera size={12} />{" "}
                  {s.newImagePreview ? "Change Photo" : "Upload Photo"}
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleServiceImage(i, e)}
                    style={{ display: "none" }}
                  />
                </label>
              </div>
        ...
        >
          ...
          >
            ...
            >
              Manage logos, wide banners, and photos
            </p>
          </div>
          <div style={fieldStyle}>
            ...
            >
              {existingGallery.map((g) => (
                <div key={g.publicId} style={{ position: "relative" }}>
                  ...
                  <button
                    type="button"
                    onClick={() => removeExistingGallery(g.publicId)}
                    style={{
                      position: "absolute",
                      top: -6,
                      right: -6,
        ...
        >
          ...
          >
            ...
            >
              Add a frequently asked question about your business
            </p>
          </div>
          <div style={fieldStyle}>
            ...
            <input
              ...
              onChange={(e) => setField("infoQuestion", e.target.value)}
            />
          </div>
          <div style={{ ...fieldStyle, marginBottom: 0 }}>
            ...
            <textarea
              style={{ ...inputStyle, resize: "vertical" }}
              rows={2}
              value={form.infoAnswer}
              onChange={(e) => setField("infoAnswer", e.target.value)}
            />
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            >
              Update your 4-digit security PIN (optional)
            </p>
          </div>
  ...
  return (
    <div
      style={{
        background: "var(--color-canvas-white)",
        border: "1px solid var(--color-subtle-ash)",
        borderLeft: "4px solid var(--color-deep-fern-green)",
        borderRadius: 12,
        padding: 20,
        fontFamily: "var(--font-pp-neue-montreal)",
        position: "relative",
          ...
          <div
            ...
            style={{
              ...
              color: rev.phone
                ? "var(--color-deep-fern-green)"
                : "var(--color-rich-black)",
              cursor: rev.phone ? "pointer" : "default",
              textDecoration: rev.phone ? "underline" : "none",
              display: "inline-flex",
              alignItems: "center",
              gap: 4,
            }}
